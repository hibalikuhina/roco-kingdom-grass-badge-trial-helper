...
#   图鉴 = database    图标 = icon        相似度 = similarity
#   领先度 = stand-out  剪贴板 = clipboard
STRINGS: dict[str, tuple[str, str]] = {
    # ---------------------------------------------------------- shell / app
    "app.title": ("洛克王国 草系徽章之旅 助手",
    ...
    Ctrl+V   在当前图鉴里查一下 - 回答 Y 或 N，并把你的输入和
             图鉴中最接近的那个图标并排显示出来
    Ctrl+N   录入当前图鉴 - 会先查一遍，如果图鉴里已经有相似的，
             会把两个都显示出来，问你是添加、替换还是取消。
             如果粘贴的是一整张网格截图，会自动切成一行一个，
             凡是图鉴中已有相似图标的行都等你选一边：你的图标
             （作为新的添加）还是已有的那个（跳过你的）。程序不会
             替你做主 - 只要还有一行没决定，导入按钮就一直是灰的
    Ctrl+E   编辑图鉴：所有图标的缩略图，可重命名、可多选删除
    F5       从磁盘重新载入图鉴
    F1       本帮助

这三件事在窗口下方都有按钮，也都在“图鉴”和“图标”菜单里。

判定会给出两个数字。“相似度”是图鉴中最接近的那个图标有多像；
“领先度”是它比图鉴里其余图标高出多少。判定为 Y 需要两个都达到阈值，
阈值可以在右上角调 - 每个图鉴分别保存。""",
                  """Copy an icon to the clipboard (any screenshot crop of one icon,
any resolution), then:
...
right - they are saved per database."""),
    ...
    "common.and_more": ("\n  ……还有 {count} 个", "\n  ... and {count} more"),
    "scores": ("相似度 {sim:.3f}   领先度 {rel:.3f}",
               "similarity {sim:.3f}   stand-out {rel:.3f}"),
    "db.summary": ("图鉴 {id:03d}   -   {count} 个图标",
                   "DB {id:03d}   -   {count} icon(s)"),

    # ---------------------------------------------------------- DB chooser
    "dbchooser.title": ("选择图鉴", "Select icon database"),
    "dbchooser.heading": ("图鉴列表", "Databases"),
    "dbchooser.new_id": ("新图鉴编号：", "New database id:"),
    "dbchooser.row": ("图鉴 {id:03d}    {count} 个图标", "DB {id:03d}    {count} icon(s)"),
    "dbchooser.pick_title": ("请选择图鉴", "Select a database"),
    "dbchooser.pick_body": ("请选中一个图鉴，或者新建一个。", "Pick a database, or create one."),
    "dbchooser.cannot_open": ("无法打开", "Cannot open"),
    "dbchooser.invalid_title": ("编号无效", "Invalid id"),
    "dbchooser.invalid_body": ("图鉴编号必须是数字。", "The database id must be a number."),
    "dbchooser.exists_title": ("已存在", "Already exists"),
    "dbchooser.delete_title": ("删除图鉴", "Delete database"),
    "dbchooser.delete_body": ("确定删除图鉴 {id:03d} 以及其中的每一个图标？\n此操作无法撤销。",
                              "Delete DB {id:03d} and every icon in it?\nThis cannot be undone."),

    # ------------------------------------------------------ similar dialog
    "similar.title": ("图鉴中已有相似的图标", "Similar icon already in the database"),
    "similar.headline": ("最接近的已有图标得分 {score:.3f}（{id}  {name}）。",
                         "Best existing match scores {score:.3f} ({id}  {name})."),
    "similar.sub": ("图鉴里的图标应当各不相同 - 新的这个真的不一样吗？",
    ...
    "batch.sub": ("勾选左边的图标表示把它作为新条目添加；勾选右边表示保留图鉴里已有的那个，跳过你这个。",
    ...
    "batch.already_in": ("图鉴中已有", "already in the database"),
    "batch.looks_similar": ("与已有图标相似", "looks similar to"),
    "batch.skip_keep": ("跳过 - 保留 {id} {name}", "skip - keep {id} {name}"),
    "batch.nothing_similar": ("图鉴中没有相似的", "nothing similar stored"),
    ...
    # -------------------------------------------------------------- editor
    "editor.title": ("编辑图鉴 {id:03d}", "Edit DB {id:03d}"),
    ...
    "editor.empty": ("这个图鉴里还没有图标。", "No icons here yet."),
    "editor.no_match": ("没有符合筛选条件的图标。", "Nothing matches the filter."),
    "editor.rename_one": ("请恰好勾选一个图标来重命名（也可以直接双击它）。",
                          "Tick exactly one icon to rename (or double-click it)."),
    "editor.delete_pick": ("请先勾选要删除的图标。", "Tick the icons you want to delete."),
    "editor.delete_title": ("删除图标", "Delete icons"),
    "editor.delete_body": ("确定从图鉴 {id:03d} 删除 {count} 个图标？\n\n{names}{more}\n\n此操作无法撤销。",
                           "Delete {count} icon(s) from DB {id:03d}?\n\n{names}{more}\n\n"
                           "This cannot be undone."),

    # --------------------------------------------------------- main window
    "main.no_db": ("未选择图鉴", "No database"),
    "main.switch_db": ("切换图鉴…", "Switch DB..."),
    "main.open_folder": ("打开文件夹", "Open folder"),
    "main.rel_label": ("领先度 ≥", "stand-out >="),
    "main.sim_label": ("相似度 ≥", "similarity >="),
    "main.btn_check": ("查询剪贴板图标  (Ctrl+V)", "Check clipboard icon  (Ctrl+V)"),
    "main.btn_add": ("把剪贴板图标录入图鉴  (Ctrl+N)", "Add clipboard icon to DB  (Ctrl+N)"),
    "main.btn_edit": ("编辑图鉴  (Ctrl+E)", "Edit database  (Ctrl+E)"),
    ...
    # --------------------------------------------------------------- menus
    "menu.database": ("图鉴", "Database"),
    "menu.switch_create": ("切换 / 新建图鉴…", "Switch / create database..."),
    "menu.edit_db": ("编辑图鉴（重命名 / 删除图标）…",
                     "Edit database (rename / delete icons)..."),
    "menu.open_folder": ("打开图鉴文件夹", "Open database folder"),
    "menu.reload": ("从磁盘重新载入", "Reload from disk"),
    "menu.quit": ("退出", "Quit"),
    "menu.icon": ("图标", "Icon"),
    "menu.check": ("用图鉴查询剪贴板里的图标", "Check clipboard icon against the database"),
    "menu.add": ("把剪贴板里的图标录入图鉴", "Add clipboard icon to the database"),
    ...
    # ------------------------------------------------------- rename / name
    "delete.one_body": ("确定从图鉴 {dbid:03d} 移除 {id}（{name}）？",
    ...
    # ------------------------------------------------------------ verdicts
    "verdict.empty_db": ("N  -  图鉴是空的。按 Ctrl+N 把这个图标录入。",
                         "N  -  the database is empty.  Press Ctrl+N to add this icon."),
    "verdict.yes": ("Y  -  打过了，图鉴中已有：{name}      {numbers}",
                    "Y  -  already in the DB:  {name}      {numbers}"),
    "verdict.no": ("N  -  没打过，图鉴中没有。Ctrl+N 可录入。      {numbers}",
                   "N  -  not in the DB.  Ctrl+N adds it.      {numbers}"),
    "verdict.similar_found": ("发现相似的图标：{name}   相似度 {sim:.3f}   领先度 {rel:.3f}",
    ...
    # ------------------------------------------------------------ statuses
    "status.opened_db": ("已打开图鉴 {id:03d}（{count} 个图标）。   {hint}",
                         "Opened DB {id:03d} ({count} icons).   {hint}"),
    "hint.use_keys": ("Ctrl+N 录入剪贴板图标，Ctrl+V 查询。",
                      "Ctrl+N adds the clipboard icon, Ctrl+V checks one."),
    "hint.empty_db": ("这个图鉴是空的 - 复制一个图标，按 Ctrl+N 录入。",
                      "This database is empty - copy an icon and press Ctrl+N to add it."),
    "status.reloaded": ("已重新载入图鉴 {id:03d}。", "Reloaded DB {id:03d}."),
    "status.thresholds": ("现在判定为 Y 需要相似度 ≥ {sim:.2f} 且领先度 ≥ {rel:.2f}。",
    ...
    "status.best": ("最接近 {id} {name} {sim:.3f}{runner_up}   （图鉴中位数 {median:.3f}）",
    ...
    # ------------------------------------------------------------ database
    "db.exists": ("图鉴 {id} 已经存在：{path}", "database {id} already exists at {path}"),
    "db.not_found": ("{root} 下没有编号为 {id} 的图鉴", "no database {id} under {root}"),
    ...
    "cli.desc": ("命令行版本：用图片文件代替剪贴板。\n\n"
                 ...
                 "    python -m iconmatch.cli remove --db 1 0003"),
    "cli.help.root": ("图鉴所在的文件夹", "database root folder"),
    "cli.help.lang": ("界面语言：zh（简体中文）或 en（English）",
                      "interface language: zh (Simplified Chinese) or en (English)"),
    "cli.help.list": ("列出所有图鉴", "list databases"),
    "cli.help.match": ("用图鉴查询图片文件", "match image files against a database"),
    "cli.help.add": ("把图片文件录入图鉴", "add image files to a database"),
    "cli.help.remove": ("按编号删除一个图标", "remove an icon by id"),
    "cli.help.name": ("只在单个文件时有意义", "only meaningful with a single file"),
    "cli.help.create": ("图鉴不存在时自动新建", "create the database if missing"),
    ...
    "cli.no_dbs": ("{root} 下没有图鉴", "no databases under {root}"),
    "cli.db_row": ("图鉴 {id:03d}  {count} 个图标", "DB {id:03d}  {count} icon(s)"),
    "cli.no_db": ("{root} 下没有编号为 {id} 的图鉴（加 --create 可以新建）",
                  "no database {id} under {root} (use --create)"),
    "cli.match_empty": ("{file}: N（图鉴是空的）", "{file}: N (database is empty)"),
    ...
    "cli.skipped": ("  {name}: 已跳过 - 与 {id} {best} 太相似"
                    "（相似度={sim:.3f} 领先度={rel:.3f}）；要照样录入请加 --force",
    ...
    # ---------------------------------------------------------- entry point
    "main.help.root": ("存放 db_XXX 图鉴文件夹的目录",
